...
Every `get_*_jac()` function returns a struct containing both a residual field (`.res`) and its partial derivatives (`.d_E_*`). The residual kernel reads only `.res`; the smoother reads both. Because both paths call the same function, the Jacobian is the exact linearization of the residua**l** (modulo the frozen-$K$ approximation in the diffusion term).
:::
...
The enthalpy flux $HuE$ has the structure of a *mass flux times a specific quantity*. Two approaches are possible for discretizing this:

1.  **Naive conserved-variable approach.** Treat $HE$ as a single conserved variable and apply a standard Riemann solver (e.g., Rusanov/Lax-Friedrichs) to it directly: $$
    F^{\text{naive}} = \tfrac{1}{2}u(H_L E_L + H_R E_R) - \tfrac{1}{2}\alpha(H_R E_R - H_L E_L).
    $$
2.  **Factored mass-flux approach.** Compute the mass flux $F^m$ first, then upwind the specific enthalpy $E$ using that mass flux as the wave speed: $$
    F = F^m \cdot \hat{E}(E_L, E_R).
    $$

The naive approach violates a fundamental consistency requirement identified by Abgrall (1996): *a uniform scalar must remain uniform regardless of what the thickness does*. To see why, suppose $E$ is uniform ($E_L = E_R = E_0$) but $H$ varies across a face. The naive dissipation term $-\tfrac{1}{2}\alpha(H_R E_R - H_L E_L) = -\tfrac{1}{2}\alpha E_0 (H_R - H_L)$ is nonzero — the $H$ gradient alone generates spurious numerical diffusion of enthalpy. In the factored form, $F = F^m \cdot E_0$ regardless of the thickness gradient. The dissipation acts only on $\nabla E$, never on $\nabla H$.

This is not a minor technicality. At ice margins, calving fronts, and grounding lines, $H$ has sharp gradients while the enthalpy profile may be smooth. The naive flux would smear the temperature field at every such feature.

### Connection to Riemann Solvers

The theoretical justification comes from the wave structure of the coupled thickness–tracer system:

$$
\partial_t H + \partial_x(Hu) = \ldots, \qquad \partial_t(HE) + \partial_x(HuE) = \ldots
$$

This is the shallow water equations with a passive tracer. In the full wave decomposition, $E$ is carried only by the *contact wave* (the middle wave, speed $u$) — it does not participate in the gravity waves that carry $H$ jumps. The Riemann solution for $E$ is therefore trivial: whichever side the mass comes from, that is the $E$ value crossing the face.

The **HLLC** (Harten-Lax-van Leer-Contact) Riemann solver was designed precisely to capture this structure. It restores the contact wave that the simpler HLL/Rusanov solvers miss, and for a passive tracer the HLLC flux naturally factors into $F_{HE} = F^m \cdot E^*$, where $E^*$ is the upwind value of $E$ across the contact.

The flux used here can be understood as: start from the HLLC framework (which separates mass flux from tracer transport), then use a Rusanov-level approximation for the tracer upwinding. The factored structure — which is the important part — comes from the HLLC wave decomposition; the specific choice of dissipation (Rusanov, Roe, etc.) is secondary. This factored mass-flux approach is standard practice in ocean models (MOM6, ROMS) and atmospheric dynamical cores (FV3), where tracers like temperature and salinity are always upwinded by the mass flux rather than treated as independent conserved variables (see also Toro 2009, Ch. 10).

### Per-Face Flux

At each face and sigma level $k$, the mass flux $F^m = \tfrac{1}{2}(H_L + H_R) \, u_{\text{face},k}$ is formed from the layer velocity at that face and the arithmetic average of the two neighboring cell thicknesses. This is the same mass-flux stencil used by the thickness solver in `flux.cu`, ensuring that the energy transport is exactly consistent with the mass transport.
...
$$
R^{\text{h-adv}} = \rho_i \, \frac{F_R - F_L + F_B - F_T}{\Delta x}
$$

where $F_R, F_L$ are the right and left x-face fluxes and $F_B, F_T$ are the bottom and top y-face fluxes (in grid index space).

At domain boundaries, a free-outflow condition is applied: the exterior ghost cell is assigned $E_{\text{ghost}} = E_{\text{here}}$ and $H_{\text{ghost}} = H_{\text{here}}$, so the Lax-Friedrichs flux reduces to $F = F^m E_{\text{here}}$ (pure outflow, no artificial dissipation).
...
The one place where mass-flux consistency is approximate rather than exact is across the operator split: the momentum solver advances $H^{n-1} \to H^n$ first, then the enthalpy solver uses $H^n$ for its fluxes. The mass fluxes that *produced* $H^n$ used intermediate Newton states during the momentum solve, not exactly $H^n$ itself. This is the standard $O(\Delta t)$ operator-splitting error and vanishes under time-step refinement.
...
The scaled vertical velocity $\omega = H\dot{\sigma}$ is an input to the enthalpy solver, not computed by it. The enthalpy kernels receive $\omega_{i,j,k}$ as a precomputed $(N_y, N_x, N_z)$ array alongside the horizontal velocity fields $u_{i,j,k}$ and $v_{i,j,k}$. How $\omega$ is produced depends on the momentum model driving the simulation.
...
with boundary values $\omega_0 = -\dot{m}$ (bed) and $\omega_{N_z-1} = -\dot{a}$ (surface). Any $\omega$ field satisfying this constraint is consistent with the conservative enthalpy equation.

### The `compute_omega` Kernel

A convenience kernel `compute_omega` is provided for the common case where $\omega$ is diagnosed from a given velocity field. It integrates the continuity equation upward from the bed:

1.  **Layer-wise mass flux divergence.** At each sigma level $k$, the horizontal mass flux divergence $\nabla_h \cdot (H\mathbf{u})_k$ is computed using the same face-averaged stencil as the horizontal enthalpy flux — $F^m_{\text{face}} = \tfrac{1}{2}(H_L + H_R) \, u_{\text{face},k}$ — ensuring exact consistency between the enthalpy equation and mass conservation. Note that each sigma level $k$ uses its own velocity $(u_k, v_k)$, so with a non-SSA momentum model the divergence varies with depth.

2.  **Column mass balance.** The vertically averaged divergence $\overline{\nabla_h \cdot (H\mathbf{u})}$ is combined with the surface and basal mass balance rates to estimate the thickness tendency:

$$
\frac{\partial H}{\partial t} \approx \dot{a} - \dot{m} - \overline{\nabla_h \cdot (H\mathbf{u})}.
$$
...
By construction, the surface value satisfies $\omega_{N_z-1} = -\dot{a}$, matching the kinematic boundary condition derived in the continuous formulation. Under SSA (depth-uniform velocities), the mass flux divergence is constant across layers, and $\omega$ is linear in $\sigma$ — the classical $\dot{\sigma} = (\dot{m} - \sigma \dot{a})/H$. Under a higher-order model with vertical shear, the layer-wise divergences differ and $\omega$ is a more general function of $\sigma$.
...
## 11. Outer Iteration and Convergence

One call to the column smoother applies the Newton solve independently to every column in parallel. The outer iteration loop then:

1.  Updates the global solution: $E \leftarrow E + \omega_{\text{relax}} \, \delta E$.
2.  Recomputes the **full** residual $R$ (including horizontal coupling from all neighbors at their updated values).
3.  Checks convergence: $\|R\|_\infty / \|R_0\|_\infty < \text{rtol}$ or $\|R\|_\infty < \text{atol}$.

Because horizontal neighbors are frozen during each sweep, the smoother is a block-Jacobi iteration. Each sweep propagates information one cell horizontally, so convergence of horizontal coupling requires $O(\max(N_x, N_y))$ sweeps in the worst case. The iteration cap `n_iter` and `absolute_tolerance` act as safety nets — the solver does its best and moves on, exactly as the momentum solver does when its maximum V-cycle count is reached.
