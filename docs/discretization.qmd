...
$$
F^m_{\text{face}} = \tfrac{1}{2}(H_L + H_R) \, u_{\text{face},k} - \tfrac{1}{2}|u_{\text{face},k}|(H_R - H_L).
$$

Note that each sigma level $k$ uses its own velocity $(u_k, v_k)$, so with a non-SSA momentum model the divergence varies with depth. Boundary faces (domain edges) have zero flux.
