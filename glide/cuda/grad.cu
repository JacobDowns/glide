...
// Gradient w.r.t. the per-cell regularized-Coulomb u_c field.
extern "C" __global__
void compute_gradient_u_c(
    float* __restrict__ grad_u_c,
    const float* __restrict__ u,
    const float* __restrict__ v,
    const float* __restrict__ H,
    const float* __restrict__ lambda_u,
    ...
    int ny, int nx, int stride, int halo)
{
	...
	if (has_u){
	    ...
	    if (j>0     )  {atomicAdd(&grad_u_c[i * nx + j - 1],lambda_u_l * j_tau_bx.d_u_c_l);}
	    if (j<(nx-1))  {atomicAdd(&grad_u_c[i * nx + j]    ,lambda_u_l * j_tau_bx.d_u_c_r);}
 	}
	...
	if (has_v){
	    ...
	    if (i>0     ) {atomicAdd(&grad_u_c[(i-1) * nx + j],lambda_v_t * j_tau_by.d_u_c_t);}
	    if (i<(ny-1)) {atomicAdd(&grad_u_c[i * nx + j]    ,lambda_v_t * j_tau_by.d_u_c_b);}
	}
    }
}

// Gradient w.r.t. the GLOBAL Weertman exponent m. Mirrors compute_gradient_beta,
// but m is a single scalar, so every momentum facet's adjoint-weighted d_m is
// reduced into grad_m[0] (rather than distributed to per-cell beta gradients).
extern "C" __global__
void compute_gradient_m(
    ...
    int ny, int nx, int stride, int halo)
{
	...
	if (has_u){
	    ...
	    float lambda_u_l = get_vfacet(lambda_u,i,j,ny,nx);
	    atomicAdd(&grad_m[0], lambda_u_l * j_tau_bx.d_m);
 	}
	...
	if (has_v){
	    ...
	    float lambda_v_t = get_hfacet(lambda_v,i,j,ny,nx);
	    atomicAdd(&grad_m[0], lambda_v_t * j_tau_by.d_m);
	}
    }
}
