...
DualFloat get_diva_drag_coeff(
    ...
    // Basal drag COEFFICIENT c(U), i.e. |tau_b| = c(U)*U, as a function of the basal speed,
    // returned as a dual so c'(U) (hence f'(U)) falls out of the same evaluation.  Serves both
    // the closure Newton in compute_diva_coeffs and the Jacobian/transpose, for every law.
    //
