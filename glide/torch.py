...
class GlideStepB(torch.autograd.Function):
    """Differentiable forward step that ALSO exposes the ice-stiffness field B
    (rheology) as an input, returning dJ/dB alongside the other gradients.

    Identical to GlideStep except B is set on the model each forward and its
    adjoint gradient is computed in backward (via model.backward(compute_B_grad=
    True) -> compute_gradient_B). Kept as a separate Function so phase-1/2 code
    that calls GlideStep is unaffected.

    Signature: forward(t, dt, model, level, H_prev, bed, beta, B, smb).
    """

    @staticmethod
    def forward(ctx, t, dt, model, level, H_prev, bed, beta, B, smb):
        ctx.t = t
        ctx.dt = dt
        ctx.model = model
        ctx.level = level
        ...
        model.mg.geometry.bed.set(cp.asarray(bed.data), start_level=level)
        model.mg.sliding.beta.set(cp.asarray(beta.data), start_level=level)
        model.mg.rheology.B.set(cp.asarray(B.data), start_level=level)
        model.mg.forcing.smb.set(cp.asarray(smb.data), start_level=level)
        ...
        ctx.save_for_backward(u_torch, v_torch, H_torch, mask_torch, phi_torch,
                              H_prev, bed, beta, B, smb)
        ctx.mark_non_differentiable(mask_torch)
    ...
    def backward(ctx, gu, gv, gH, gM):
        ...
        model = ctx.model
        level = ctx.level
        (u_torch, v_torch, H_torch, mask_torch, phi_torch,
         H_prev, bed, beta, B, smb) = ctx.saved_tensors
        ...
        converged = model.backward(t, dt, dJdu=cp.asarray(gu), dJdv=cp.asarray(gv),
                                   dJdH=cp.asarray(gH), compute_B_grad=True)

        g_H_prev = torch.tensor(model.mg[level].state.H_prev.grad)
        g_bed = torch.tensor(model.mg[level].geometry.bed.grad)
        g_beta = torch.tensor(model.mg[level].sliding.beta.grad)
        g_B = torch.tensor(model.mg[level].rheology.B.grad)
        g_smb = torch.tensor(model.mg[level].forcing.smb.grad)
        ...
        return None, None, None, None, g_H_prev, g_bed, g_beta, g_B, g_smb
