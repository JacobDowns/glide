...
Checks that the transposed DIVA operator can actually be solved -- i.e. that
compute_vjp_diva and vanka_smooth_adjoint_diva form a consistent pair and the FAS
...
def solve(stress_balance, vcycles=20):
    mg = build(stress_balance)
    grid = mg.levels[0]
...
def main():
    for tag, sb in (("SSA ", 0.0), ("DIVA", 1.0)):
        r = solve(sb)
