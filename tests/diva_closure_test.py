...
value known independently rather than against a stored regression:
  ...
  6. idempotence -- repeated calls must agree, which an unconverged closure does not.

This is a UNIT test of the kernel's root-finding at a GIVEN regularization, so the membrane
(eps_reg) and shear (eps_reg_shear) regularizers are set equal here -- that is the single value
the float64 reference assumes.  The physics of driving eps_reg_shear -> 0 (F2 -> the analytic SIA
value) is diva_slab_test's job, not this one.
...
def build(glen_n=GLEN_N):
    ...
    for lvl in mg.levels:                                        # DIVA closure config, per level
        lvl.rheology.n_sigma.set(cp.float32(N_SIGMA))
        lvl.rheology.eps_reg_shear.set(cp.float32(EPS_REG))     # aligned with the reference below
...
def reference_closure(beta, glen_exp):
    ...
    """
    A = EPS_REG                      # membrane strain rates vanish; shear reg == EPS_REG (build)
...
def check_shear_dominated(glen_exp):
    ...
    # 0.67), and the shear-dominated asymptotic guess carries an n in its exponent.
...
def main():
    ...
    # integrates a quadratic exactly with 2 nodes and so certainly with N_SIGMA of them.
    quad = 1.0 / 3.0
